...
class HubSpotService {
  ...
  // Buscar contacto por teléfono (para integración WhatsApp)
  ...
  // Crear nota/actividad en contacto
  async createNote(contactId, note, engagementType = 'NOTE') {
    try {
      const response = await axios.post(`${this.baseURL}/crm/v3/objects/notes`, {
        properties: {
          hs_note_body: note,
  ...
  async getContactDeals(contactId) {
    try {
      const response = await axios.get(`${this.baseURL}/crm/v4/objects/contacts/${contactId}/associations/deals`, {
        headers: this.getHeaders()
      });
      return response.data;
    } catch (error) {
